...
class TaskFlowPro:
    ...
    def button(self, parent, text, command, bg=None):
        ...
        return tk.Button(
            ...
            command=command,
            bg=bg,
            fg="white" if bg != self.card else self.text,
            activebackground=self.accent_hover if bg != self.card else self.input,
            activeforeground="white" if bg != self.card else self.text,
            relief="flat",
            bd=0,
            font=("Segoe UI", 9, "bold"),
            padx=13,
    ...
    def build(self):
        ...
        tk.Label(
            title_box,
            text="A modern local productivity workspace",
            font=("Segoe UI", 10),
            bg=self.bg,
            fg=self.muted,
        ...
        self.combo(
            add,
            self.category,
            ["Personal", "Work", "Study", "Shopping", "Health", "Project"],
            11,
        ).pack(side="left", padx=7)
        self.due = tk.StringVar()
        de = self.entry(add, "YYYY-MM-DD")
        de.configure(textvariable=self.due)
        de.pack(side="left", width=115, ipady=10, padx=7)
        self.button(add, "＋ Add Task", self.add_task, self.accent).pack(side="left")
        ...
        self.combo(
            tools,
            self.cat_filter,
            ["All", "Personal", "Work", "Study", "Shopping", "Health", "Project"],
            12,
        ...
        for c in cols:
            ...
            self.tree.column(
                c,
                width=widths[c],
                anchor="center" if c in ("id", "done", "favorite", "priority", "due") else "w",
            )
        ...
        for text, cmd, color in [
            ("✓ Complete", self.complete_selected, self.success),
            ("✎ Edit", self.edit_selected, self.accent),
            ("★ Favorite", self.favorite_selected, self.warning),
            ("Delete", self.delete_selected, self.danger),
            ("Clear Done", self.clear_completed, "#7c3aed"),
        ]:
        ...
        tk.Label(
            bottom,
            text="Developed by Roshab Bhandari",
            font=("Segoe UI", 9),
            bg=self.bg,
            fg=self.muted,
    ...
    def add_task(self):
        ...
        if due and due != "YYYY-MM-DD":
            ...
            due = ""
        self.db.add_task(title, "", self.category.get(), "", self.priority.get(), due)
    ...
    def edit_selected(self):
        ...
        for label, key in [
            ...
            if key == "description":
                ...
                w = self.combo(d, var, ["Personal", "Work", "Study", "Shopping", "Health", "Project"] if key == "category" else ["Low", "Medium", "High"], 15)
        ...
        def save():
            ...
            self.db.update_task(t["id"], title, description, category, tags, priority, due)
            d.destroy()
            self.refresh()
    ...
    def refresh(self):
        ...
        rows = self.db.get_tasks(
            self.search.get() if hasattr(self, "search") else "",
            self.status.get() if hasattr(self, "status") else "All",
            self.cat_filter.get() if hasattr(self, "cat_filter") else "All",
            self.pri_filter.get() if hasattr(self, "pri_filter") else "All",
            self.favorites.get() if hasattr(self, "favorites") else False,
            self.sort.get() if hasattr(self, "sort") else "created_at DESC",
        )
        for t in rows:
            tags = []
            if t["completed"]:
        ...
        s = self.db.stats()
        keys = ["total", "completed", "pending", "high", "favorite", "overdue"]
    ...
    def reminder_check(self):
        try:
            ...
            if overdue:
                self.root.title(f"Roshab Tasks • {overdue} overdue task(s)")
            else:
                self.root.title("Roshab Tasks")
        finally:
            self.root.after(60000, self.reminder_check)
