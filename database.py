...
class Database:
    ...
    def init_db(self):
        with self.connect() as conn:
            ...
            for name, definition in (("reminder_at", "TEXT DEFAULT ''"), ("reminder_enabled", "INTEGER DEFAULT 0"), ("reminder_sent", "INTEGER DEFAULT 0")):
                if name not in columns:
                    conn.execute(f"ALTER TABLE tasks ADD COLUMN {name} {definition}")

    def add_task(self, title, description, category, tags, priority, due_date, reminder_at="", reminder_enabled=0):
        now = datetime.now().isoformat(timespec="seconds")
        with self.connect() as conn:
            cur = conn.execute("""INSERT INTO tasks
                (title,description,category,tags,priority,due_date,reminder_at,reminder_enabled,reminder_sent,created_at,updated_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
                (title,description,category,tags,priority,due_date,reminder_at,int(reminder_enabled),0,now,now))
    ...
    def update_task(self, task_id, title, description, category, tags, priority, due_date, reminder_at="", reminder_enabled=0):
        now = datetime.now().isoformat(timespec="seconds")
        with self.connect() as conn:
            old = conn.execute("SELECT reminder_at FROM tasks WHERE id=?", (task_id,)).fetchone()
            if old is None or old[0] != reminder_at:
                conn.execute("""UPDATE tasks SET title=?,description=?,category=?,tags=?,priority=?,due_date=?,reminder_at=?,reminder_enabled=?,reminder_sent=0,updated_at=? WHERE id=?""",
                    (title,description,category,tags,priority,due_date,reminder_at,int(reminder_enabled),now,task_id))
            else:
                conn.execute("""UPDATE tasks SET title=?,description=?,category=?,tags=?,priority=?,due_date=?,reminder_at=?,reminder_enabled=?,updated_at=? WHERE id=?""",
                    (title,description,category,tags,priority,due_date,reminder_at,int(reminder_enabled),now,task_id))
    ...
    def stats(self):
        with self.connect() as conn:
            ...
            reminders=conn.execute("SELECT COUNT(*) FROM tasks WHERE completed=0 AND reminder_enabled=1 AND reminder_at<>'' AND reminder_at >= datetime('now','localtime')").fetchone()[0]
            return total,completed,total-completed,high,favorites,overdue,reminders

    def due_reminders(self, now=None):
        now = now or datetime.now().strftime("%Y-%m-%d %H:%M")
        with self.connect() as conn:
            return conn.execute("""SELECT * FROM tasks
                WHERE completed=0 AND reminder_enabled=1 AND reminder_sent=0
                AND reminder_at<>'' AND reminder_at <= ?
                ORDER BY reminder_at ASC""", (now,)).fetchall()

    def mark_reminder_sent(self, task_id):
        with self.connect() as conn:
            conn.execute("UPDATE tasks SET reminder_sent=1,updated_at=? WHERE id=?", (datetime.now().isoformat(timespec="seconds"), task_id))
